...
class NYCamera
{
	public:
		...
		/**
		  ...
		  * - la position de la camera : _Position
		  * - le point regarde par la camera : _LookAt
		  * - la vecteur up de notre monde : _UpRef
		  */
		...
		{
			_LookAt -= _Position;
			_LookAt.rotate(_UpRef, angle);//rotation se fait par rapport au Z du monde, pas celui de la camera( sinon ça merde )
			_LookAt += _Position;
			updateVecs();			 
		}
